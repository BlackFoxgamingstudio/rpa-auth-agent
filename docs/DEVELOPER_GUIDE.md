# Developer Guide: Sovereign RPA Auth Agent
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/rpa-auth-agent.git
cd rpa-auth-agent
...
```bash
export SBB_RPA_TARGET_URL=your_value_here
export SBB_RPA_USERNAME=your_value_here
export SBB_RPA_PASSWORD=your_value_here
export SBB_MCP_SERVER_URL=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-rpa-auth-agent)
...
```bash
# Build
docker build -t sovereign-rpa-auth-agent:dev .

# Run with env file
docker run --env-file .env -p 8788:8788 sovereign-rpa-auth-agent:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-rpa-auth-agent
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8788`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-rpa-auth-agent",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8788` in use | `lsof -i :8788` | Kill PID or change `SERVICE_PORT` |
